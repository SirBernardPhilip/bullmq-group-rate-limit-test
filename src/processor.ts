...
import { SubtaskPayloadInterface, BackgroundJobs, QueueBroker } from './queue-broker';
import { RateLimiter } from './rate-limiter';

export const processor: ProcessorPro<SubtaskPayloadInterface, void, BackgroundJobs> = async (
	job
) => {
	...
	if (jobPayload.subtaskId === 'subtask2' && RateLimiter.getInstance().shouldRateLimit()) {
		console.log('Rate limiting job with data: ', jobPayload);
		QueueBroker.getInstance().workers['IMPORT_EVERNOTE'].rateLimitGroup(job, 5000);
