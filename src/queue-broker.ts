...
export class QueueBroker {
	private static instance: QueueBroker;

	public static getInstance(): QueueBroker {
		if (!QueueBroker.instance) {
			QueueBroker.instance = new QueueBroker();
		}
		return QueueBroker.instance;
	}

	private readonly queues: {
		[key in BackgroundJobs]: QueuePro<SubtaskPayloadInterface, void, BackgroundJobs>;
	};
	public readonly workers: {
		[key in BackgroundJobs]: WorkerPro<SubtaskPayloadInterface, void, BackgroundJobs>;
	};
	constructor() {
		this.queues = {} as {
			[key in BackgroundJobs]: QueuePro<SubtaskPayloadInterface, void, BackgroundJobs>;
		...
		for (const jobType of Object.values(JobTypeEnum)) {
			this.queues[jobType] = new QueuePro<SubtaskPayloadInterface, void, BackgroundJobs>(
				jobType,
				{
					connection: {
						host: url.hostname,
			...
			this.workers[jobType] = new WorkerPro<SubtaskPayloadInterface, void, BackgroundJobs>(
				jobType,
				processor,
				{
					connection: {
